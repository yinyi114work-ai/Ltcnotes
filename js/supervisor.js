...
function loadSupervisorTasks(){
  try{
    ...
    supervisorTasks = Array.isArray(raw) ? raw.filter(item=>item && item.id) : [];
  }catch(e){ supervisorTasks = []; }
}
function saveSupervisorTasks(){
  localStorage.setItem(SUPERVISOR_STORAGE_KEY, JSON.stringify(supervisorTasks));
}
...
function openSupervisorForm(task){
  ...
  $('supervisorTaskTime').value = task?.time || '';
  $('supervisorTaskPriority').value = task?.priority || 'normal';
  $('supervisorTaskStatus').value = task?.status || 'pending';
  $('supervisorTaskNote').value = task?.note || '';
  $('supervisorTaskSubject').focus();
  form.scrollIntoView({behavior:'smooth', block:'nearest'});
}
function closeSupervisorForm(){
  const form = $('supervisorTaskForm');
  if(form) form.hidden = true;
  if($('supervisorTaskId')) $('supervisorTaskId').value = '';
}
function saveSupervisorTask(){
  const subject = v('supervisorTaskSubject');
  const date = v('supervisorTaskDate');
  ...
  const id = v('supervisorTaskId');
  const existing = supervisorTasks.find(task=>task.id===id);
  const item = {
    id: id || `task_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,
    type: v('supervisorTaskType') || 'other',
    subject,
    date,
    time: v('supervisorTaskTime'),
    priority: v('supervisorTaskPriority') || 'normal',
    status: v('supervisorTaskStatus') || 'pending',
    note: v('supervisorTaskNote'),
    createdAt: existing?.createdAt || Date.now(),
    updatedAt: Date.now()
  };
  if(existing) supervisorTasks = supervisorTasks.map(task=>task.id===id ? item : task);
  else supervisorTasks.push(item);
  saveSupervisorTasks();
  closeSupervisorForm();
  renderSupervisorDashboard();
  showToast(existing ? '已更新待辦' : '已新增待辦');
}
function toggleSupervisorTask(id, checked){
  supervisorTasks = supervisorTasks.map(task=>task.id===id ? {...task, status:checked ? 'done' : 'pending', updatedAt:Date.now()} : task);
  saveSupervisorTasks();
  renderSupervisorDashboard();
}
function deleteSupervisorTask(id){
  ...
  supervisorTasks = supervisorTasks.filter(item=>item.id!==id);
  saveSupervisorTasks();
  renderSupervisorDashboard();
  showToast('已刪除待辦');
}
function isSupervisorOverdue(task){
  return task.status !== 'done' && task.date && task.date < supervisorDateStr(new Date());
}
function renderSupervisorStats(){
  const today = supervisorDateStr(new Date());
  const pending = supervisorTasks.filter(t=>t.status!=='done');
  const urgent = pending.filter(t=>t.priority==='urgent').length;
  const todayCount = pending.filter(t=>t.date===today).length;
  const overdue = pending.filter(isSupervisorOverdue).length;
  $('supervisorStats').innerHTML = `
    <div class="supervisor-stat"><span>急件</span><strong>${urgent}</strong></div>
    <div class="supervisor-stat"><span>今日待辦</span><strong>${todayCount}</strong></div>
    <div class="supervisor-stat"><span>已逾期</span><strong>${overdue}</strong></div>`;
}
function renderSupervisorTasks(){
  ...
  if(!tasks.length){
    $('supervisorTaskList').innerHTML = `<div class="supervisor-empty">${supervisorSelectedDate ? '這一天目前沒有待辦事項。' : '目前沒有待辦事項，按「新增待辦」開始使用。'}</div>`;
    return;
  }
  $('supervisorTaskList').innerHTML = tasks.map(task=>{
    const type = SUPERVISOR_TYPE_META[task.type] || SUPERVISOR_TYPE_META.other;
    const statusLabel = task.status==='progress' ? '處理中' : task.status==='done' ? '已完成' : '待處理';
    ...
    return `<article class="supervisor-task priority-${task.priority} ${overdue?'is-overdue':''} ${task.status==='done'?'is-done':''}" data-id="${supervisorEscape(task.id)}">
      ...
      <div class="task-main">
        <div class="task-title-row"><span class="task-title">${supervisorEscape(task.subject)}</span><span class="task-type-badge">${type.icon} ${type.label}</span><span class="task-status-badge ${task.status}">${statusLabel}</span></div>
        <div class="task-meta"><span>📅 ${supervisorEscape(task.date)}</span>${task.time?`<span>🕒 ${supervisorEscape(task.time)}</span>`:''}<span>${overdue?'⚠ 已逾期':`優先：${SUPERVISOR_PRIORITY_META[task.priority]?.label || '一般'}`}</span></div>
        ${task.note?`<p class="task-note">${supervisorEscape(task.note)}</p>`:''}
      </div>
...
function renderSupervisorCalendar(){
  const year = supervisorCalendarCursor.getFullYear();
  const month = supervisorCalendarCursor.getMonth();
  ...
  for(let i=0;i<42;i++){
    ...
    cells.push(`<button class="calendar-day ${d.getMonth()!==month?'other-month':''} ${ds===today?'today':''} ${ds===supervisorSelectedDate?'selected':''}" type="button" data-date="${ds}">
      <span class="calendar-day-number">${d.getDate()}</span><span class="calendar-dots">${priorities.map(p=>`<i class="calendar-dot ${p}"></i>`).join('')}</span></button>`);
  }
  $('supervisorCalendar').innerHTML = cells.join('');
  document.querySelectorAll('#supervisorCalendar .calendar-day').forEach(btn=>btn.addEventListener('click',()=>{
    supervisorSelectedDate = btn.dataset.date;
    const selected = new Date(`${supervisorSelectedDate}T00:00:00`);
    supervisorCalendarCursor = new Date(selected.getFullYear(), selected.getMonth(), 1);
    renderSupervisorDashboard();
  }));
}
...
function initSupervisorTool(){
  ...
  loadSupervisorTasks();
  supervisorCalendarCursor = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
