...
function renderMutationSpecific(){
  ...
  const html = {
    ...
    quota: `<div class="card"><h3 class="card-title">額度開立回報</h3><div class="grid-2">
      <label>適用月份（僅供填寫時確認）<input id="mutQuotaMonth" type="month"></label>
      <label>補充說明<input id="mutQuotaNote" placeholder="例如：依上月實際服務使用情形彙整"></label>
    </div><p class="small-note">請輸入本期實際使用的服務碼別與單位數，系統會協助整理成可回報個管師的文字。</p><div class="button-row"><button id="addMutQuotaRow" class="secondary-btn" type="button">＋新增碼別</button></div><div id="mutQuotaRows" class="mut-row-list"></div><p id="mutQuotaCalc" class="small-note"></p></div>`,
...
function addMutQuotaRow(code='BA07', count=1){
  ...
  wrap.innerHTML = `<label>碼別<select class="mut-quota-code">${serviceOptionHtml()}</select></label><label>使用單位數<input class="mut-quota-count" type="number" min="0" step="1" value="${count}"></label><button class="remove-row" type="button">刪除</button>`;
...
function generateMutation(){
  ...
  if(type === 'pause'){
    ...
    const totalUnits = rows.reduce((sum,x)=>sum+x.count,0);
    const totalAmount = rows.reduce((sum,x)=>sum+x.subtotal,0);
    const details = rows.map(x=>`${x.code}${x.svc.name}，共使用${x.count}單位。`).join('
') || '未填寫碼別及單位數。';
