...
const environmentOptions = ['環境整潔','動線安全','浴室防滑需留意','採光通風良好','有跌倒風險','有輔具／環境改善需求','暫無新增環境改善需求'];
const incidentOptions = ['跌倒','急診','住院','拒絕服務','家屬抱怨','失智遊走','居服員反映異常','無特殊異常事件'];
const needOptions = ['希望增加服務','希望減少服務','輔具／環境改善需求','短照需求','暫無新增需求'];
...
function updateFee(){
  ...
  let total = 0;
  let remainingQuotaForCalc = quota;
  let coveredAmount = 0;
  let overage = 0;

  document.querySelectorAll('.fee-row').forEach(row=>{
    const code = row.querySelector('.fee-code').value;
    const count = Math.max(0, Math.floor(Number(row.querySelector('.fee-count').value || 0)));
    const service = serviceData.find(s=>s.code===code);
    if(!service || count <= 0) return;

    total += service.price * count;

    // 實務邏輯：剩餘額度若不足以完整支應「一單位」服務，該單位不拆算，整筆列為自費。
    const coveredUnits = Math.min(count, Math.floor(remainingQuotaForCalc / service.price));
    const selfPayUnits = count - coveredUnits;
    const rowCoveredAmount = coveredUnits * service.price;
    const rowOverageAmount = selfPayUnits * service.price;

    coveredAmount += rowCoveredAmount;
    overage += rowOverageAmount;
    remainingQuotaForCalc -= rowCoveredAmount;
  });

  const selfPay = Math.floor(coveredAmount * rate);
  const clientTotal = selfPay + overage;
  const remain = Math.max(remainingQuotaForCalc, 0);

  const selectedServices = Array.from(document.querySelectorAll('.fee-row')).map(row=>{
    const code = row.querySelector('.fee-code').value;
    return serviceData.find(s=>s.code===code);
  }).filter(Boolean);
  const uniqueSelectedServices = Array.from(new Map(selectedServices.map(s=>[s.code, s])).values());
  const insufficientServices = uniqueSelectedServices.filter(s=>remain < s.price);
  ...
  if(overage > 0){
    ...
    notice.textContent = `⚠️ 已有服務單位無法由 CMS 額度完整支應，需自費 ${money(overage)} 元；預估自付總額為 ${money(clientTotal)} 元。`;
  }else{
    notice.className = 'notice-card success';
    notice.textContent = `✅ 目前已選服務皆可由 CMS 額度完整支應，剩餘額度 ${money(remain)} 元；預估自付總額為 ${money(clientTotal)} 元。`;
  }

  const risk = $('feeRisk');
  if(risk){
    if(total <= 0){
      risk.className = 'notice-card';
      risk.textContent = '請新增服務碼別與每月次數後，系統會協助判斷剩餘額度是否足以支應下一次服務。';
    }else if(remain === 0){
      risk.className = 'notice-card danger';
      risk.textContent = '⚠️ 剩餘額度為 0 元，下一次服務將無法由 CMS 額度支應，可能需自費或請個管師協助開立額度。';
    }else if(insufficientServices.length){
      const list = insufficientServices.slice(0,3).map(s=>`${s.code}${s.name}（${money(s.price)}元/${s.unit}）`).join('、');
      const more = insufficientServices.length > 3 ? `等 ${insufficientServices.length} 個碼別` : '';
      risk.className = 'notice-card danger';
      risk.textContent = `⚠️ 剩餘額度 ${money(remain)} 元，不足以完整支應 ${list}${more} 下一次服務；若再使用該碼別，該次服務可能需自費或請個管師協助開立額度。`;
    }else{
      risk.className = 'notice-card success';
      risk.textContent = '✅ 目前剩餘額度仍足以完整支應已選碼別的下一次服務。';
    }
  }
}
